    ...
    Experiment& error(T& sketch) {
        ...
        for (const auto& p: E -> cnt) 
            ...
            for (int i = 0; i < RE_CDF_SIZE; ++i)
                if (re <= i)
                    re_cdf[i] += 1.0;
            E -> are[str] = re / p.second;
            E -> aae[str] = ae / p.second;
            for (int i = 0; i < RE_CDF_SIZE; ++i)
                if (re <= i)
